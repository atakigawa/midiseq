...
Sequencer = function(midiMgr, n, c) {
  ...
  this.Draw = function() {
      ...
      for (var i = 1; i <= 6; i++) {
        ($('#'+this.rangeid)[0])[i-1]  = new Option(i,i,i==3,i==3);
      }
      for (var i = 1; i <= 16;i++) {
        ($('#'+this.chanid)[0])[i-1]   = new Option(i,i,i==me.channel,i==me.channel);
      }
      for (var i = 1; i <= 24; i++) {
        ($('#'+this.trigid)[0])[i-1]   = new Option(i,i,i==me.trigger,i==me.trigger);
      }
      for (var i = -48; i <= 48; i++) {
        ($('#'+this.transid)[0])[48-i] = new Option(i,i,i==me.transpose,i==me.transpose);
      }
      ...
      for (i = 0; i < this.notes.length; i++) {
        ...
        for (var j = 127; j >= 1; j--) {
          sid[127-j] = new Option(notestr(j),j,j==this.notes[i],j==this.notes[i]);
        }
        ...
        for (var j = 1; j <= 8; j++) {
          sid[j-1] = new Option(j,j,j==this.repeat[i],j==this.repeat[i]);
        }
        ...
        for (var j = 0; j <= 24; j++) {
          sid[j] = new Option(j,j,j==this.noteoff[i],j==this.noteoff[i]);
        }
  ...
  this.Refresh = function() {
    for (i = 0; i < this.notes.length; i++) {
      var sid = $('#not'+this.seqid+i)[0];
      for (var j = 127; j >= 1; j--) {
        sid[127-j].selected = j==this.notes[i];
      }
      var sid = $('#rep'+this.seqid+i)[0];
      for (var j = 1; j <= 8; j++) {
        sid[j-1].selected = j==this.repeat[i];
      }
      var sid = $('#retr'+this.seqid+i)[0];
      sid.checked = this.retrig[i];
      var sid = $('#noff'+this.seqid+i)[0];
      for (var j = 0; j <= 24; j++) {
        sid[j].selected = j==this.noteoff[i];
      }
    }
    this.totsteps();
  }
...
Player = function(midiMgr) {
  ...
  this.setdefaults = function() {
    ...
    if (this.midi_out == "Apple DLS Synth" ||
        this.midi_out == "Microsoft GS Wavetable Synth" ) {
      seq01.SetDefault();
      seq02.SetDefault();
    }
  }
